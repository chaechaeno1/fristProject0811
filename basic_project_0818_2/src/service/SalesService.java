...
public class SalesService {
	...
	public int salesYear() {
		...
		for (Map<String, Object> item : list) {
			...
			sec++;
			totalCnt += Integer.parseInt(String.valueOf(item.get("CNT")));
			totalSal += Integer.parseInt(String.valueOf(item.get("SAL")));
		}
		System.out.println("총 계 : \t\t" + totalCnt + " 명, \t\t" + totalSal);
		return View.MANAGE_SALES;
	}

	public int salesMonth() {
		System.out.print(">>조회할 달 입력 (yyMM) : ");
		String date = ScanUtil.nextLine();
		List<Object> param = new ArrayList<>();
		List<Map<String, Object>> list = null;
		...
		for (Map<String, Object> item : list) {
			...
			System.out.printf("%2d : " + item.get("RT"), sec);
			System.out.print(" \t " + item.get("CNT"));
			System.out.println(" \t " + item.get("SAL"));
			sec++;
			totalCnt += Integer.parseInt(String.valueOf(item.get("CNT")));
	...
	public int salesDay() {
		System.out.print(">>조회할 일자 입력 (yyMMdd) : ");
		String date = ScanUtil.nextLine();
		List<Object> param = new ArrayList<>();
		List<Map<String, Object>> list = null;
