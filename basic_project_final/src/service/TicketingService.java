...
public class TicketingService {
	...
	public int searchingStation() {
		...
		System.out.print("출발지 번호를 입력하세요 >>");

		int start_st = ScanUtil.nextInt();
		if (start_st == 0) {
		...
		System.out.println("되돌아가기 : 0");
		System.out.print("목적지 번호를 입력하세요 >>");

		int end_st = ScanUtil.nextInt();
		if (end_st == 0) {
			System.out.println("되돌아갑니다...");
			return View.TICKET;
		}
		...
		try {
			ticketStorage.put("start_station", list.get(start_st - 1).get("ST_ID"));
			ticketStorage.put("end_station", list.get(end_st - 1).get("ST_ID"));
	...
	public int searchingRoute() {
		...
		param = new ArrayList<>();
		param.add(ticketStorage.get("start_station"));
		param.add(ticketStorage.get("end_station"));
		...
		if (list == null) {
			...
			System.out.println("==============================================================================");
			System.out.println("번호    \t   노선       시간 \t\t\t    버스번호 \t등급");
			System.out.println("==============================================================================");
			int sec = 1;

			for (Map<String, Object> item : list) {

				System.out.printf("%2d : %8s ", sec, item.get("RT_NAME"));
	...
	public int buy() {
		...
		while (true) {
			if (seatCount != 1) {
				...
				System.out.print("어린이의 숫자를 입력 >> ");
				littlePerson = ScanUtil.nextInt();
				if (littlePerson > seatCount || littlePerson < 0) {
					System.out.println("잘못 입력하셨습니다.");
					continue;
				} else
					break;
			} else {
				Map<String, Object> cstAge = dao.getAge(logParam);
				if (Integer.parseInt(String.valueOf(cstAge.get("CST_AGE"))) <= 13)
					littlePerson++;
				break;
			}
		}
		...
		for (Object item : seatParam) {
			if (ltmp > 0) {
				totalPrice += calcPrice.price((int) ticketStorage.get("price"), (String) ticketStorage.get("grade"),
						"어린이");
				ltmp--;
			} else {
				totalPrice += calcPrice.price((int) ticketStorage.get("price"), (String) ticketStorage.get("grade"),
		...
		int bill_flag = dao.bill(billParam);
		int tk_flag = 0;
		...
		for (int i = 0; i < seatParam.size(); i++) {
			...
			tk_flag += dao.buyTicket(ticketParam);
		}
		
		if(bill_flag != 0 && tk_flag != 0) {
	...
	public int ticketList() {
		...
		if (list == null) {
			...
			int sec = 1;
			for (Map<String, Object> item : list) {
				System.out.printf(" %2d : %10s\t", sec, item.get("RT_NAME"));
				String str = String.valueOf(item.get("SV_DATE"));
				System.out.print(" " + str.substring(0, 16));
				System.out.printf(" \t%10s", item.get("BUS_NUM"));
				System.out.printf(" \t%6s", String.valueOf(item.get("TK_SNUM")));
				System.out.printf(" \t%5s", item.get("AGE_DIV"));
				str = String.valueOf(item.get("RES_DATE"));
				System.out.printf(" \t " + str.substring(0, 16));
				if (item.get("BUS_GRADE").equals("p"))
